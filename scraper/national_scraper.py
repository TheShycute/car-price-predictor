# national_scraper.py - 全国二手车爬虫 (可中断续传)
import asyncio, csv, re, os, time
from playwright.async_api import async_playwright

CHROME_DATA = r"F:\Program Files\codex\codexwork\chrome_data"
DATA_DIR = r"F:\Program Files\codex\codexwork\car-price-predictor\data"
CSV_PATH = os.path.join(DATA_DIR, "cars_raw.csv")
STATE_PATH = os.path.join(DATA_DIR, "scrape_state.txt")
...
def parse_listings(text):
    ...
    for i, line in enumerate(lines):
        ...
        if m:
            ...
            if name:
                results.append({
                    "name": name, "year": year, "mileage": mileage,
...
async def scrape_city(page, pinyin, name, max_pages_per_range=50):
    city_data = []
    seen_local = set()
    for range_label, price_param in PRICE_RANGES:
        ...
        no_more = False
        empty = 0
        while not no_more and pg <= max_pages_per_range and empty < 2:
...
async def main():
    ...
    print(f"Loaded {len(all_data)} records. Resuming from city #{start_idx}")
    ...
    async with async_playwright() as p:
        ...
        for idx in range(start_idx, len(TOP_CITIES)):
            pinyin, name = TOP_CITIES[idx]
            existing = sum(1 for r in all_data if r.get("city") == name)
            print(f"[{idx+1}/{len(TOP_CITIES)}] {name} (existing: {existing})", flush=True)
